...
So far we used just the English as language recognition package. However, many more languages are supported.
...
Now we download and load the chinese langauge trained data

```{r}
# Only need to do download once:
tesseract_download("chi_tra")
# Now load the dictionary
Chinese <- tesseract("chi_tra")
```
...
First we download the japanese data
...
```{r}
Chinese<- tesseract("chi_tra")
ChineseJap<- tesseract(c("chi_tra", "jpn"))
ChinJapEng<- tesseract(c("chi_tra", "jpn", "eng"))
...
Now let's tray with another image
